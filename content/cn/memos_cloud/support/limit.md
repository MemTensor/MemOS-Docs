...
| searchMemory   | 40,000 token     | 事实记忆：25 条<br>偏好记忆：25 条<br>工具记忆：25 条<br>技能：25 条 |

另外，知识库文档上传相关功能存在以下限制：

- 支持上传的文档类型：PDF、DOCX、DOC、TXT、JSON、MD、XML
- 单文件大小上限：不超过 100 MB、500 页
- 单次上传的文件数量上限：不超过 20 个
...
- 每分钟请求输入 token 总量不超过 400,000 token，超出后请求将被限流。
