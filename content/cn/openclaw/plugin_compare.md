...
| 🔍&nbsp;**检索能力** | 云端语义向量检索 | FTS5 全文 + 向量混合召回（RRF + MMR + 时间衰减） |
| 🧠&nbsp;**记忆进化** | 暂不支持 | 碎片对话自动归纳为结构化任务，任务完成后触发技能评估，自动生成或升级可复用 Skill |
| 👥&nbsp;**多&nbsp;Agent** | ✅ 支持（`multiAgentMode`，数据隔离） | ✅ 支持（内存隔离 + 公共记忆 + 技能共享） |
| 💡&nbsp;**额外能力** | • Token 成本降低 60%<br>• 自动记录所有对话<br>• 用户偏好专门分类 | • 全量记忆可视化（Web 管理面板，7 个页面）<br>• 原生记忆一键导入<br>• 分级模型配置（为不同任务分配不同模型） |
...
| 场景 | 推荐方案 | 理由 |
| --- | --- | --- |
| 个人开发者，重视隐私 | 本地插件 | 数据不上云，完全自主控制，可离线运行 |
| 处理敏感数据（医疗、金融、法律等） | 本地插件 | 数据不出本地，符合合规要求 |
| 离线 / 内网隔离环境 | 本地插件 | 可配置本地 Embedding 模型，无任何网络依赖 |
| 需要技能进化与任务管理 | 本地插件 | 独有的任务生成与技能进化流水线，Agent 越用越聪明 |
| 本地开发调试，需可视化管理 | 本地插件 | 内置 Memory Viewer，记忆 / 任务 / 技能全量透明可控 |
| 团队协作 / 多设备办公 | 云插件 | 跨设备记忆同步，无需手动迁移 |
| 多 Agent 跨机器共享记忆 | 云插件 | 云端天然支持跨设备同步与记忆隔离 |
| 快速上手，不想折腾环境 | 云插件 | 无需安装编译工具，配置 API Key 即可用 |
| 临时使用 / 试用 | 云插件 | 即开即用，无需本地资源占用 |
