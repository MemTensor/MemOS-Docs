---
title: Local Plugin
desc: Use @memtensor/memos-local-plugin to bring local-first long-term memory, three-tier retrieval, skill crystallization, and an observable management panel to OpenClaw, Hermes Agent, and DeepSeek Harness.
---

::note
**Compatibility recommendation**
Use a version of OpenClaw, Hermes Agent, or DeepSeek Harness supported by the current MemOS Local Plugin release to avoid plugin loading or runtime errors.
::

`@memtensor/memos-local-plugin` is the new MemOS local plugin: one local-first memory core for **OpenClaw**, **Hermes Agent**, and **DeepSeek Harness**. It does not host your memory data in the cloud. Instead, it maintains SQLite data, skill packages, and logs on your own machine so the agent can accumulate reusable experience locally.

If you want a cloud-hosted memory service with the simplest API Key setup, see the [Cloud Plugin](/openclaw/guide). If you care more about privacy, local runtime, observability, or using the same local memory capability across OpenClaw, Hermes, and DeepSeek Harness, use this local plugin.

## Core Capabilities

| Capability | Description |
| --- | --- |
| Local-first | OpenClaw, Hermes, and DeepSeek Harness each get an isolated runtime home. SQLite, skills, logs, and config stay on your machine. |
| Multi-agent support | OpenClaw integrates through an in-process TypeScript plugin; Hermes integrates through a Python Provider that talks to the same Node.js memory core over JSON-RPC; DeepSeek Harness integrates through its native plugin mechanism. |
...
| Import and migration | Supports JSON import/export, legacy plugin migration, and agent-specific native imports for OpenClaw session JSONL, Hermes `MEMORY.md`, or DeepSeek Harness session data. |
...
| --- | --- | --- |
| 1. Agent adapter | OpenClaw, Hermes, and DeepSeek Harness send conversations, tool calls, and feedback to the shared `MemoryCore` through their adapters. | Standardized turns, tool outcomes, feedback |
...
The installer auto-detects whether OpenClaw, Hermes, and/or DeepSeek Harness are installed. In an interactive terminal, it asks which agent to install for; in non-interactive environments, it installs for the detected agent(s). It deploys plugin code, installs production dependencies, and restarts the target runtime when needed.
...
| DeepSeek Harness | `http://127.0.0.1:18801` |

If you install OpenClaw, Hermes, and DeepSeek Harness together, they use separate Viewers and separate local data directories.
...
| DeepSeek Harness | Run `npx @deepseek-ai/dsh web` | DeepSeek Harness adapter calls `MemoryCore` |
...
4. **Import**: migrate legacy data, import OpenClaw session JSONL, Hermes `MEMORY.md`, DeepSeek Harness session data, or import/export JSON backups.
5. **Help**: look up field meanings such as `V`, `α`, `R_human`, `η`, support, and gain.

## Agent Differences

| Item | OpenClaw | Hermes | DeepSeek Harness |
| --- | --- | --- | --- |
| Integration | TypeScript plugin, in-process calls to `MemoryCore` | Python `MemoryProvider`, stdio JSON-RPC to Node bridge | DeepSeek Harness adapter calls `MemoryCore` |
| Default Viewer | `http://127.0.0.1:18799` | `http://127.0.0.1:18800` | Provided by the DeepSeek Harness runtime |
| Model configuration | Configure in OpenClaw Viewer Settings → AI Models | Configure in Hermes Viewer Settings → AI Models | Configure in the DeepSeek Harness Web UI under Settings → Models |
| Data sharing | Isolated from Hermes and DeepSeek Harness by default | Isolated from OpenClaw and DeepSeek Harness by default | Isolated from OpenClaw and Hermes by default |

All three agents use separate databases and Viewers even when installed on the same machine. Team sharing only starts after you explicitly enable `hub:`.

## Available Tools

OpenClaw, Hermes, and DeepSeek Harness expose memory tools through their own host interfaces. Common capabilities include:
