---
title: 本地插件
desc: 使用 @memtensor/memos-local-plugin 为 OpenClaw、Hermes Agent 与 DeepSeek Harness 提供本地优先的长期记忆、三层检索、技能结晶和可观测管理面板。
---

::note
**兼容性建议**
请使用当前版本支持的 OpenClaw、Hermes Agent 或 DeepSeek Harness 版本，以避免插件加载或运行错误。
::

`@memtensor/memos-local-plugin` 是 MemOS 新一代本地插件：一套本地优先的记忆核心，同时支持 **OpenClaw**、**Hermes Agent** 与 **DeepSeek Harness**。它不会把记忆数据托管到云端，而是在你的机器上维护 SQLite 数据库、技能包和日志，让 Agent 在本地持续积累可复用经验。

如果你只想快速接入云端托管记忆，请查看[云插件](/cn/openclaw/guide)。如果你更看重隐私、本机运行、可观测性，或希望 OpenClaw、Hermes 与 DeepSeek Harness 都使用同一套本地记忆能力，请使用本页面的本地插件。

## 核心能力

| 能力 | 说明 |
| --- | --- |
| 本地优先 | OpenClaw、Hermes 与 DeepSeek Harness 各自拥有独立运行目录，SQLite、Skill、日志和配置都保留在本机。 |
| 多 Agent 支持 | OpenClaw 通过 TypeScript 插件进程内接入；Hermes 通过 Python Provider + JSON-RPC 桥接到同一套 Node.js 记忆核心；DeepSeek Harness 通过其原生的插件机制接入。 |
...
| 导入与迁移 | 支持 JSON 导入导出、旧版插件数据迁移，以及按当前 Agent 导入 OpenClaw 会话 JSONL、Hermes `MEMORY.md` 或 DeepSeek Harness 对应的会话数据。 |
...
| 阶段 | 发生了什么 | 产物 |
| --- | --- | --- |
| 1. Agent 接入 | OpenClaw、Hermes 与 DeepSeek Harness 通过各自 Adapter 把会话、工具调用和反馈交给统一的 `MemoryCore`。 | 标准化的 turn、tool outcome、feedback |
...
安装器会自动检测系统中是否已安装 OpenClaw、Hermes 或 DeepSeek Harness。交互式终端会询问安装到哪个 Agent；非交互环境会自动安装到检测到的 Agent。安装器会部署插件代码、安装生产依赖，并在需要时重启对应运行时。
...
如果你同时安装了 OpenClaw、Hermes 和 DeepSeek Harness，它们会使用各自独立的 Viewer 和本地数据目录。
...
| DeepSeek Harness | 运行 `npx @deepseek-ai/dsh web` | DeepSeek Harness adapter 调用 `MemoryCore` |
...
4. **Import**：导入旧版数据、OpenClaw 会话 JSONL、Hermes `MEMORY.md`、DeepSeek Harness 对应的会话数据，或导入 / 导出 JSON 备份。
5. **Help**：查看每个字段含义，例如 `V`、`α`、`R_human`、`η`、support、gain 等。

## Agent 差异

| 项目 | OpenClaw | Hermes | DeepSeek Harness |
| --- | --- | --- | --- |
| 接入方式 | TypeScript 插件，进程内调用 `MemoryCore` | Python `MemoryProvider`，通过 stdio JSON-RPC 调用 Node bridge | 通过 DeepSeek Harness adapter 接入 `MemoryCore` |
| 默认 Viewer | `http://127.0.0.1:18799` | `http://127.0.0.1:18800` | 由 DeepSeek Harness 运行时提供 |
| 模型配置 | 在 OpenClaw Viewer 的 Settings → AI Models 中配置 | 在 Hermes Viewer 的 Settings → AI Models 中配置 | 在 DeepSeek Harness Web UI 的 Settings → Models 中配置 |
| 数据共享 | 默认与 Hermes、DeepSeek Harness 隔离 | 默认与 OpenClaw、DeepSeek Harness 隔离 | 默认与 OpenClaw、Hermes 隔离 |

三个 Agent 即使安装在同一台机器上，也会使用各自的数据库和 Viewer。只有显式开启 `hub:` 后，才会进行团队共享。

## 可用工具

OpenClaw、Hermes 和 DeepSeek Harness 会通过各自宿主暴露记忆工具，常见能力包括：
